  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">采购申请</h1><p className="text-sm text-zinc-500">申请 · 审批 · 购买 · 入库 · 报表</p></div>
        {isStaff && <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-sky-600"><Plus className="h-4 w-4"/>申请采购</button>}
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div className="rounded-xl border p-3 bg-white dark:bg-zinc-900 text-center"><div className="text-xl font-bold text-green-600">{stats.received}</div><div className="text-xs text-zinc-400">已入库</div></div>
          <div className="rounded-xl border p-3 bg-white dark:bg-zinc-900 text-center"><div className="text-xl font-bold text-zinc-700">¥{stats.totalSpent.toFixed(0)}</div><div className="text-xs text-zinc-400">总支出</div></div>
        </div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-zinc-900 rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={submitRequest} className="px-4 py-2 rounded-lg text-sm bg-sky-500 text-white hover:bg-sky-600">提交申请</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-1 rounded-xl bg-zinc-100 dark:bg-zinc-800 p-1">
        {[
          ...
          <button key={t.key} onClick={() => { setTab(t.key); if (t.key==="all") fetchAll(); if (t.key==="report") fetchReport(); else fetchData(); }}
            className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === t.key ? "bg-white dark:bg-zinc-700 shadow-sm" : "text-zinc-500 hover:text-zinc-700"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab !== "report" && (
        <div className="space-y-3">
          {requests.length === 0 ? <div className="rounded-xl border bg-white dark:bg-zinc-900 p-12 text-center text-zinc-400">暂无采购申请</div> : requests.map(r => {
            const steps = ["pending", "approved", "purchased", "received"];
            const currentStep = r.status === "rejected" ? -1 : steps.indexOf(r.status);
            return (
            <div key={r.id} className="rounded-xl border bg-white dark:bg-zinc-900 p-5 space-y-4">
              {/* Header */}
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {r.reason && <div className="text-sm text-zinc-400 mt-1">📝 {r.reason}</div>}
                  {r.rejectReason && <div className="text-sm text-red-500 mt-1">❌ {r.rejectReason}</div>}
                </div>
              </div>
                    ...
                    return (
                      <div key={i} className="flex-1 flex items-center">
                        <div className={`flex flex-col items-center flex-1 ${i > 0 ? "" : ""}`}>
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${done ? (active ? "bg-sky-500 text-white ring-4 ring-sky-100 dark:ring-sky-900" : "bg-green-500 text-white") : "bg-zinc-200 dark:bg-zinc-700 text-zinc-400"}`}>
                            {done && !active ? "✓" : i + 1}
                          </div>
                          <span className={`text-[11px] mt-1 font-medium ${done ? "text-zinc-700 dark:text-zinc-300" : "text-zinc-400"}`}>{label}</span>
                        </div>
                        {i < 3 && (
              ...
              <div className="flex gap-2 pt-1 border-t border-zinc-100 dark:border-zinc-800">
                {r.status === "pending" && isAdmin && (
                  <div className="flex gap-2 w-full">
                    <button onClick={()=>approve(r.id)} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-green-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-green-600 transition-colors"><Check className="h-4 w-4"/>批准采购</button>
                    <button onClick={()=>reject(r.id)} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-red-50 border border-red-200 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-100 transition-colors"><X className="h-4 w-4"/>拒绝</button>
                  </div>
                )}
                  ...
                  <div className="w-full space-y-2">
                    {purchaseId === r.id ? (
                      <div className="flex gap-2 items-center">
                        <span className="text-sm text-zinc-500 shrink-0">实际金额 ¥</span>
                        ...
                        <button onClick={markPurchased} className="px-4 py-2 rounded-lg bg-purple-500 text-white text-sm font-medium hover:bg-purple-600 shrink-0">确认</button>
                        <button onClick={()=>{setPurchaseId(null);setPurchasePrice("");}} className="px-3 py-2 rounded-lg border text-sm shrink-0">取消</button>
                      </div>
                    ) : (
                      <button onClick={()=>setPurchaseId(r.id)} className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-purple-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-purple-600 transition-colors"><ShoppingCart className="h-4 w-4"/>标记已购买</button>
                    )}
                  </div>
                )}
                {r.status === "purchased" && isStaff && (
                  <button onClick={()=>markReceived(r.id)} className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-green-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-green-600 transition-colors"><PackageCheck className="h-4 w-4"/>确认收货并入库存</button>
                )}
                {r.status === "received" && (
                  <div className="w-full text-center text-sm text-green-600 font-medium py-2">✅ 已完成入库，物品已加入库存</div>
                )}
                {r.status === "rejected" && (
                  <div className="w-full text-center text-sm text-red-500 font-medium py-2">此申请已被拒绝</div>
                )}
              </div>
            </div>
          )})}
      ...
      {tab === "report" && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <button onClick={fetchReport} className="px-4 py-2 rounded-lg text-sm bg-sky-500 text-white hover:bg-sky-600">查询</button>
          </div>

          {report && (
            <>
              <div className="grid grid-cols-4 gap-3">
                <div className="rounded-xl border p-4 bg-white dark:bg-zinc-900"><div className="text-2xl font-bold">{report.totalRequests}</div><div className="text-xs text-zinc-400">总申请</div></div>
                <div className="rounded-xl border p-4 bg-white dark:bg-zinc-900"><div className="text-2xl font-bold text-green-600">{report.approvedCount}</div><div className="text-xs text-zinc-400">已批准</div></div>
                <div className="rounded-xl border p-4 bg-white dark:bg-zinc-900"><div className="text-2xl font-bold text-red-600">{report.rejectedCount}</div><div className="text-xs text-zinc-400">已拒绝</div></div>
