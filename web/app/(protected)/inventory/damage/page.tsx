    ...
    <div className="space-y-4 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted">记录物料异常情况，按管理办法追责</p>
        </div>
      ...
      <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-1.5 rounded-lg bg-danger px-3 py-2 text-sm font-medium text-white hover:bg-danger shadow-sm w-fit">
        <Plus className="h-4 w-4" /> 报备损坏/遗失
      </button>

      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        {loading ? <div className="p-8 text-center text-faint">加载中...</div> :
         reports.length === 0 ? <div className="p-8 text-center text-faint"><ShieldAlert className="h-8 w-8 mx-auto mb-2 opacity-30" />暂无报备记录</div> :
              ...
              <div className="flex items-start justify-between">
                ...
                {r.liability === "pending" && isAdmin && (
                  <div className="flex items-center gap-1 shrink-0 ml-2">
                    <button onClick={() => resolve(r.id, "exempt")} className="px-3 py-1.5 rounded-lg bg-success text-white text-xs font-medium hover:bg-success">免责</button>
                    <button onClick={() => resolve(r.id, "compensate")} className="px-3 py-1.5 rounded-lg bg-danger text-white text-xs font-medium hover:bg-danger">需赔偿</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                  placeholder="详细描述损坏/遗失情况..." className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm h-24" />
              </div>
              ...
              <button onClick={submit} className="w-full rounded-lg bg-danger px-4 py-2.5 text-sm font-medium text-white hover:bg-danger">
