...
import { GitBranch, Upload, Loader2, CheckCircle, XCircle, Clock, RefreshCw } from "lucide-react";
...
export default function WikiImportPage() {
  ...
  const statusLabel = (s: string) => {
    switch (s) { case "completed": return "已完成"; case "failed": return "失败"; case "pending": return "排队中"; case "preparing": return "准备中"; case "catalog": return "生成目录"; case "documents": return "生成文档"; default: return s; }
  };
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-800">
          {tasks.length === 0 ? <div className="p-8 text-center text-zinc-400">暂无任务</div> : tasks.map(t => (
            <div key={t.id} className="p-3 flex items-center gap-3">
              {statusIcon(t.status)}
              <div className="flex-1"><div className="font-medium text-sm">{t.projectName}</div><div className="text-xs text-zinc-500">{statusLabel(t.status)}</div>{t.errorMessage && <div className="text-xs text-red-500 mt-1">{t.errorMessage}</div>}</div>
