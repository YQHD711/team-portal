  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-sm text-muted mt-1">查看 AI 生成的代码项目文档</p>
      </div>

      {projects.length === 0 ? (
        <div className="rounded-xl border border-border bg-surface p-12 text-center">
          <BookOpen className="h-12 w-12 mx-auto mb-3 text-zinc-300 dark:text-zinc-600" />
          <p className="text-muted">暂无已完成的项目文档</p>
          <p className="text-sm text-faint mt-1">管理员和部长可以提交代码仓库来生成文档</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2">
          {projects.map(p => (
            <Link key={p.id} href={`/wiki/${p.id}`}
              className="group relative rounded-xl border border-border bg-surface p-5 hover:shadow-md hover:border-sky-300 dark:hover:border-sky-700 transition-all">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold group-hover:text-sky-600 transition-colors">{p.projectName} <span className="text-xs font-normal text-faint">{visLabel(p.visibility)}</span></h3>
