...
export default function CloudAuthPanel({ authUrl, authCode, onAuthCode, authMsg, loadingAuthUrl, onGetAuthUrl, onSubmitCode }: Props) {
  return (
    <div className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-5 space-y-4">
      ...
      {!authUrl ? (
        <button onClick={onGetAuthUrl} disabled={loadingAuthUrl} className="rounded-xl bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600 disabled:opacity-50">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={onSubmitCode} className="rounded-lg bg-green-500 px-4 py-2 text-sm font-medium text-white hover:bg-green-600">确认</button>
          </div>
