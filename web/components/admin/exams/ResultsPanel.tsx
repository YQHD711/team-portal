  ...
  return (
    <div className="border-t border-border bg-background/50 p-4 space-y-4">
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium">选择队员 * <span className="text-xs text-faint">(已录入的不再显示, 可多选批量录入)</span></label>
        {available.length === 0 ? (
          <p className="text-sm text-faint">全部队员均已录入成绩</p>
        ) : (
          <div className="max-h-36 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 gap-x-3 gap-y-1.5 rounded-lg border border-border bg-surface p-3">
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button onClick={submitBatch} disabled={busy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm bg-primary text-white hover:bg-accent-hover disabled:opacity-60">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {selResults.size > 0 && (
            <button onClick={deleteSelected} disabled={busy} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs bg-red-50 dark:bg-red-950 text-danger hover:bg-red-100 disabled:opacity-60">
              <Trash2 className="h-3.5 w-3.5" />删除所选 ({selResults.size})
            </button>
          )}
        </div>
        {loading ? (
          <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-faint" /></div>
        ) : results.length === 0 ? (
          <div className="text-center py-6 text-sm text-faint">暂无成绩记录</div>
        ) : (
            ...
            {results.map(r => (
              <div key={r.id} className="px-4 py-2.5 flex items-center justify-between gap-2 text-sm">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {r.passed ? <Check className="h-4 w-4 text-success" /> : <XCircle className="h-4 w-4 text-red-400" />}
