  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-4 py-2 text-sm font-medium text-white hover:bg-sky-600">
      ...
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 divide-y">
        {invites.length === 0 ? (
          <div className="p-12 text-center text-zinc-400"><Ticket className="h-8 w-8 mx-auto mb-2 text-zinc-300" /><p>暂无邀请码</p></div>
        ) : invites.map(c => (
          <div key={c.id} className="p-4 flex items-center justify-between">
            ...
            {!c.isRevoked && <button onClick={() => revoke(c.id)} className="text-xs text-red-500 hover:underline">作废</button>}
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-zinc-900 shadow-xl border p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-4 justify-end">
              ...
              <button onClick={generate} className="px-4 py-2 rounded-lg text-sm bg-sky-500 text-white">生成</button>
            </div>
          </div>
        </div>
      )}
