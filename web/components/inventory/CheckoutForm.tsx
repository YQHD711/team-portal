...
export default function CheckoutForm({ showNew, onOpen, onClose, allItems, itemSearch, onItemSearch, newForm, setNewForm, onSubmit }: Props) {
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      {!showNew ? (
        <button onClick={onOpen} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-white hover:bg-accent-hover shadow-sm">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-faint hover:text-zinc-600"><X className="h-4 w-4" /></button>
          </div>
            ...
            {filtered.slice(0, 20).map(item => (
              <button key={item.id}
                ...
                <div>
                  ...
                  <span className="text-xs text-faint ml-2">{item.category}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`inline-flex rounded-full px-1.5 py-0.5 text-xs font-bold ${item.grade === "A" ? "bg-red-100 text-red-700" : item.grade === "B" ? "bg-amber-100 text-amber-700" : "bg-zinc-100 text-muted"}`}>{item.grade}</span>
                  <span className="text-xs text-faint">库存: {item.quantity}</span>
                </div>
              </button>
            ))}
            {filtered.length === 0 && <div className="px-3 py-4 text-center text-sm text-faint">无匹配物料</div>}
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={onSubmit} disabled={!newForm.itemId}
              className="px-4 py-2 rounded-lg bg-primary text-white text-sm font-medium hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed">
              提交申请
            </button>
          </div>
        </div>
