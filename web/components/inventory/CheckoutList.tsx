...
export default function CheckoutList({ tab, loading, my, pending, isStaff, isAdmin, onCheckin, onApproveDept, onApproveAdmin, onReject }: Props) {
  if (tab === "my") {
    return (
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        {loading ? <div className="p-8 text-center text-faint">加载中...</div> :
         my.length === 0 ? <div className="p-8 text-center text-faint"><ArrowLeftRight className="h-8 w-8 mx-auto mb-2 opacity-30" />暂无领用记录</div> :
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`inline-flex rounded-full px-1.5 py-0.5 text-xs font-bold ${r.grade === "A" ? "bg-red-100 text-red-700" : r.grade === "B" ? "bg-amber-100 text-amber-700" : "bg-zinc-100 text-muted"}`}>{r.grade}级</span>
                  ...
                  {r.rejectReason && <div className="text-xs text-danger mt-1">驳回原因: {r.rejectReason}</div>}
                ...
                {r.status === "approved" && isStaff && (
                  <button onClick={() => onCheckin(r)} className="ml-2 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-medium hover:bg-primary shrink-0">
                    归还
                  </button>
                )}
                {r.status === "approved" && !isStaff && r.grade === "A" && (
                  <span className="ml-2 text-[11px] text-faint shrink-0 self-center">A级归还需管理员/部长操作</span>
                )}
              </div>
            </div>
          ))}
  ...
  return (
    <div className="rounded-xl border border-border bg-surface overflow-hidden">
      {pending.length === 0 ? <div className="p-8 text-center text-faint"><Clock className="h-8 w-8 mx-auto mb-2 opacity-30" />暂无待审批申请</div> :
       <div className="divide-y divide-border-subtle">
        {pending.map(r => (
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted">→ {statusLabels[r.status]}</span>
                </div>
                <div className="text-xs text-muted mt-1">
                ...
                {(r.status === "pending_dept" || (r.status === "pending_admin" && isAdmin)) && (
                  <>
                    {r.status === "pending_dept" && (
                      <button onClick={() => onApproveDept(r.id)} className="px-3 py-1.5 rounded-lg bg-success text-white text-xs font-medium hover:bg-success"><Check className="h-3 w-3 inline mr-1" />批准</button>
                    )}
                    {r.status === "pending_admin" && isAdmin && (
                      <button onClick={() => onApproveAdmin(r.id)} className="px-3 py-1.5 rounded-lg bg-success text-white text-xs font-medium hover:bg-success"><Check className="h-3 w-3 inline mr-1" />终审通过</button>
                    )}
