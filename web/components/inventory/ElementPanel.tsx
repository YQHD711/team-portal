...
export function ElementPanel({ onAdd }: ElementPanelProps) {
  return (
    <div className="w-40 shrink-0 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-3 overflow-y-auto">
      <h3 className="text-xs font-semibold text-zinc-500 mb-2">元素</h3>
        ...
        {KINDS.map(kind => {
          ...
          return (
            <button key={kind} onClick={() => onAdd(kind)} title={`添加${def.label}`}
              className="flex w-full items-center gap-2 rounded-lg border border-zinc-200 dark:border-zinc-800 px-2.5 py-2 text-sm hover:border-sky-400 hover:bg-sky-50 dark:hover:bg-sky-950/40 transition-colors">
