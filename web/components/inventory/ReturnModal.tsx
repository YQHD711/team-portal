    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => !submitting && onClose()}>
      <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-zinc-900 shadow-xl border border-zinc-200 dark:border-zinc-800 p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => !submitting && onClose()} className="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"><X className="h-5 w-5" /></button>
        </div>

        <div className="text-sm text-zinc-600 dark:text-zinc-300 mb-4">
          {target.item?.name || `物料 #${target.inventoryItemId}`}
          <span className={`ml-2 inline-flex rounded-full px-1.5 py-0.5 text-xs font-bold ${target.grade === "A" ? "bg-red-100 text-red-700" : target.grade === "B" ? "bg-amber-100 text-amber-700" : "bg-zinc-100 text-zinc-500"}`}>{target.grade}级</span>
          <span className="text-zinc-400 ml-2">× {target.quantity} 件</span>
        </div>

        {/* 照片上传 */}
        <div className="mb-4">
          <label className="block text-sm font-medium mb-1">
            归还照片 {target.grade === "A" && <span className="text-red-500">* (A级必传)</span>}
          </label>
          ...
          {photoUrl && <div className="mt-2 text-xs text-green-600 flex items-center gap-1"><Check className="h-3 w-3" />已上传: {photoName}</div>}
        </div>

        {/* 功能测试说明 */}
        <div className="mb-4">
          <label className="block text-sm font-medium mb-1">
            功能测试说明 {target.grade === "A" && <span className="text-red-500">* (A级必填)</span>}
          </label>
        ...
        <div className="mb-5">
          ...
          <div className="flex gap-2">
            <button onClick={() => onCondition("normal")}
              className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${condition === "normal" ? "bg-green-50 border-green-300 text-green-700 dark:bg-green-900/30 dark:border-green-700 dark:text-green-300" : "border-zinc-200 text-zinc-500 hover:border-zinc-300 dark:border-zinc-700"}`}>✅ 完好</button>
            <button onClick={() => onCondition("damaged")}
              className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${condition === "damaged" ? "bg-red-50 border-red-300 text-red-700 dark:bg-red-900/30 dark:border-red-700 dark:text-red-300" : "border-zinc-200 text-zinc-500 hover:border-zinc-300 dark:border-zinc-700"}`}>⚠️ 有损坏</button>
          </div>
        </div>
