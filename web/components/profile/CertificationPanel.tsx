...
export function CertificationPanel({ items, loading = false }: { items: ExamPassView[]; loading?: boolean }) {
  if (loading) return <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-zinc-400" /></div>;

  return (
    <div className="space-y-4">
      <p className="text-xs text-zinc-400 flex items-center gap-1"><BadgeCheck className="h-3.5 w-3.5" />认证由管理员/部长发起考核并通过后自动获得</p>
        ...
        {items.length === 0 ? (
          ...
          <div key={p.id} className="p-4 flex items-start justify-between">
            <div className="flex items-center gap-2">
              ...
              {p.score != null && <span className="text-xs text-zinc-500">{p.score}分</span>}
            </div>
