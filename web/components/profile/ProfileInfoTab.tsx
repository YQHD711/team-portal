      ...
      {editInfo ? (
        <>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button onClick={onSave} disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-4 py-2 text-sm font-medium text-white hover:bg-sky-600 disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}保存</button>
          </div>
        </>
      ) : (
        <>
          ...
          <div className="flex justify-end"><button onClick={onStartEdit} className="px-4 py-2 rounded-lg text-sm bg-sky-500 text-white hover:bg-sky-600">编辑档案</button></div>
        </>
      )}
    </div>
  );
